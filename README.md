...
**[Otimizador de Corte Industrial](https://github.com/caiogadotti/otimizador-corte)** &nbsp;·&nbsp; `otimização` &nbsp;·&nbsp; `visão computacional` &nbsp;·&nbsp; `público`
Motor de nesting 2D para corte de peças retangulares num rolo de tecido, nascido do encontro de duas experiências minhas: desenho técnico na Systra (AutoCAD) e corte de TNT na Descartee. Compara heurísticas de faixa e skyline com um validador estrito de sobreposição/limites que roda antes de qualquer resultado ser confiado, já que um bug de nesting normalmente faz o aproveitamento parecer *melhor*, não pior. Também lê e escreve DXF real (testado indo e voltando pelo FreeCAD), extrai peças de desenho técnico via OpenCV clássico, e inclui um primeiro passo de autocomplete de desenho assistido por IA: com só um lado desenhado, antes da peça estar pronta, sugere como ela provavelmente termina, aprendendo com um histórico do próprio usuário (não só um catálogo fixo) e respeitando quais peças não podem ser rotacionadas por causa do sentido de fabricação do tecido.

A parte interessante foi uma escolha de critério de desempate que contradiz a literatura clássica de bin packing: minimizar a área enterrada sob cada peça posicionada, a heurística padrão, fragmentava mensuravelmente o perfil do skyline e derrubava o aproveitamento de 89,6% para até 68,3% dependendo da agressividade aplicada, enquanto a regra mais simples de "menor topo resultante" venceu por manter o perfil como poucos segmentos largos em vez de muitos degraus estreitos. Feito com Python, OpenCV e ezdxf.
