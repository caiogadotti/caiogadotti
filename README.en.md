...
**[Industrial Cutting Optimizer](https://github.com/caiogadotti/otimizador-corte)** &nbsp;·&nbsp; `optimization` &nbsp;·&nbsp; `computer vision` &nbsp;·&nbsp; `public`
2D nesting engine for cutting rectangular pieces from a fabric roll, built from two of my own experiences meeting: technical drawing at Systra (AutoCAD) and TNT fabric cutting at Descartee. Compares shelf and skyline heuristics with a strict overlap/bounds validator that runs before any result is trusted, since a nesting bug typically makes the yield metric look *better*, not worse. Also reads and writes real DXF files (round-tripped through FreeCAD), extracts pieces from technical drawings via classic OpenCV, and includes a first AI-assisted drawing autocomplete step: with only one edge drawn, before the piece is finished, it suggests how it's likely to end up, learning from the user's own history (not just a fixed catalog) and respecting which pieces cannot be rotated due to fabric grain direction.

The interesting part was a tie-break choice contradicting bin-packing textbooks: minimizing the buried area under each placed piece, the standard heuristic, measurably fragmented the skyline profile and dropped yield from 89.6% to as low as 68.3% depending on how aggressively it was applied, while the simpler "lowest resulting top" rule won by keeping the profile as a few wide segments instead of many narrow steps. Built with Python, OpenCV and ezdxf.
